...
try {
    $stmt = $conn->prepare("SELECT id, username, nama_lengkap, email, role FROM users WHERE id = :id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email_form) || !filter_var($email_form, FILTER_VALIDATE_EMAIL)) $errors[] = "Email tidak valid.";
    if (!empty($password_new) && $password_new !== $konfirmasi_password_new) $errors[] = "Konfirmasi password baru tidak cocok.";
    if (!empty($password_new) && strlen($password_new) < 6) $errors[] = "Password baru minimal 6 karakter.";
    if (!in_array($role_form, ['admin', 'user'])) $errors[] = "Role tidak valid.";

    // Cek duplikasi username/email
    if (empty($errors)) {
        try {
            $stmt_check = $conn->prepare("SELECT id FROM users WHERE (username = :username OR email = :email) AND id != :id_current LIMIT 1");
            ...
            if ($stmt_check->rowCount() > 0) {
                $errors[] = "Username atau Email baru sudah digunakan oleh user lain.";
            }
        } catch (PDOException $e) {
    ...
    if (empty($errors)) {
        try {
            $sql_update = "UPDATE users SET username = :username, nama_lengkap = :nama_lengkap, email = :email, role = :role";
            $params_update = [
            ...
            if (!empty($password_new)) {
                $hashed_password_new = password_hash($password_new, PASSWORD_BCRYPT);
    ...
    <form action="edit.php?id=<?php echo $user_id; ?>" method="post">
        ...
        <div>
            ...
            <select id="role" name="role">
                <option value="user" <?php echo ($role_form === 'user' ? 'selected' : ''); ?>>User</option>
